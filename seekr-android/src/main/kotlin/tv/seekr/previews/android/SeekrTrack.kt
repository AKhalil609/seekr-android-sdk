...
class SeekrTrack internal constructor(
    ...
    suspend fun thumbnailAt(positionMs: Long): Bitmap? {
        ...
        return withContext(Dispatchers.Default) {
            runCatching {
                // The xywh fragment carries the real tile size; fall back to a 16:9 block
                // only if the VTT omitted it, so we never crop a zero-sized region.
                ...
                Bitmap.createBitmap(sheet, x, y, w, h)
            }.getOrNull()
        }
    }
}
